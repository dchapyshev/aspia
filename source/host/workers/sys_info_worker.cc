...
// The groups a feature can be shown under, one repeated field of the report each. A feature is
// listed where it is looked for and not where the vendor happened to put its bit.
enum CpuidFeatureGroup
{
...
// A bit of a CPUID register and what the processor says with it. The name comes last so the
// registers stay in columns of their own.
...
proto::system_info::Processor::Feature* addFeature(
    proto::system_info::Processor* processor, CpuidFeatureGroup group)
{
    switch (group)
    {
