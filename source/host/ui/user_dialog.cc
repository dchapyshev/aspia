...
UserDialog::UserDialog(const peer::User& user, const QStringList& exist_names, QWidget* parent)
    : QDialog(parent),
      ...
      user_(user)
{
    ...
    if (user.isValid())
    {
        ui.checkbox_disable_user->setChecked(!(user.flags & peer::User::ENABLED));
...
void UserDialog::onButtonBoxClicked(QAbstractButton* button)
{
    if (ui.button_box->standardButton(button) == QDialogButtonBox::Ok)
    {
        if (account_changed_)
        {
            ...
            if (!peer::User::isValidUserName(name))
            {
            ...
            user_ = peer::User::create(name, password);
            if (!user_.isValid())
            {
                QMessageBox::warning(this,
                                     tr("Warning"),
