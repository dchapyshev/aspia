...
bool D3dDevice::initialize(int index, const ComPtr<IDXGIAdapter>& adapter)
{
    ...
    if (!dxgi_adapter_)
    {
        LOG(ERROR) << "An empty IDXGIAdapter instance has been received";
        return false;
    }

    D3D_FEATURE_LEVEL feature_level;

    LOG(INFO) << "Try to create adapter #" << index;
    ...
    {
        LOG(ERROR) << "D3D11CreateDeivce returns error" << error.ErrorMessage() << "with code"
                   << error.Error();
        return false;
    }

    LOG(INFO) << "Adapter #" << index << "is created";
    ...
    {
        LOG(INFO) << "Adapter #" << index << ":" << adapter_description.Description
                  << "(DirectX feature level:" << featureLevelToString(feature_level) << ")";
    }
    else
    {
        LOG(ERROR) << "Unable to get adapter description";
    }

    if (feature_level < D3D_FEATURE_LEVEL_11_0)
    {
        LOG(ERROR) << "D3D11CreateDevice returns an instance without DirectX 11 support, level"
    ...
    {
        LOG(ERROR) << "ID3D11Device is not an implementation of IDXGIDevice, this usually "
...
std::vector<D3dDevice> D3dDevice::enumDevices()
{
    ...
    {
        LOG(ERROR) << "Cannot create IDXGIFactory1. Error" << error.ErrorMessage() << "with code"
                   << error.Error();
        ...
        if (error.Error() == S_OK)
        {
            ...
            for (UINT j = 0;; ++j)
            {
                ...
                else
                {
                    ...
                    {
                        LOG(INFO) << "Output" << j << "not attached to desktop for adapter #" << i;
                    }
                    else
                    {
                        adapter_used = true;
            ...
            if (!adapter_used)
            {
                LOG(INFO) << "Adapter #" << i << "not used";
                continue;
            }

            D3dDevice device;

            LOG(INFO) << "Try to initialize adapter #" << i;

            if (device.initialize(i, adapter))
            {
                result.emplace_back(std::move(device));
                LOG(INFO) << "Adapter #" << i << "is initialized";
            }
            else
            {
                LOG(ERROR) << "Unable to initialize adapter #" << i;
                return std::vector<D3dDevice>();
            }
        }
        else if (error.Error() == DXGI_ERROR_NOT_FOUND)
        {
            LOG(INFO) << "No more devices";
            break;
        }
        else
        {
            LOG(ERROR) << "IDXGIFactory1::EnumAdapters returns an unexpected error"
                       << error.ErrorMessage() << "with code" << error.Error();
        }
    }
