...
public:
    explicit UserSessionProcessProxy(std::unique_ptr<UserSessionProcess> process);
...
signals:
    void stateChanged(UserSessionProcess::State state);
    void clientListChanged(const UserSessionProcess::ClientList& client_list);
    void credentialsChanged(const proto::Credentials& credentials);
...
private:
    std::shared_ptr<base::TaskRunner> runner_;
