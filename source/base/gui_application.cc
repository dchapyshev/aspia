...
public:
    ...
    {
        // The icon of an action inside a line edit has to fit the text field, so it keeps the size
        // of the base style. Qt derives this metric from the small icon size, which would make the
        // icon as large as the ones in toolbars.
        if (metric == QStyle::PM_LineEditIconSize)
            return QProxyStyle::pixelMetric(QStyle::PM_SmallIconSize, option, widget);
