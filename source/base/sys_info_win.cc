...
QByteArray SysInfo::hardwareId()
{
    ...
    // The SMBIOS system UUID comes from the firmware and survives OS reinstallation.
    const QByteArray smbios = smbiosDump();
    if (!smbios.isEmpty())
    {
        for (SmbiosTableEnumerator enumerator(smbios); !enumerator.isAtEnd(); enumerator.advance())
        {
            const SmbiosTable* table = enumerator.table();
            if (table->type != SMBIOS_TABLE_TYPE_SYSTEM)
                continue;

            SmbiosSystem system(table);
            if (system.isValid())
            {
                QByteArray uuid = system.uuid();
                if (!uuid.isEmpty())
    ...
    // Some machines have no usable SMBIOS UUID; fall back to the GUID generated by Windows
    // at installation time. The value lives in the 64-bit registry view only.
    RegKey key;
    LONG status = key.open(HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Cryptography",
                           KEY_READ | KEY_WOW64_64KEY);
    if (status != ERROR_SUCCESS)
    {
        LOG(ERROR) << "Unable to open registry key:" << status;
        return QByteArray();
    }

    QString machine_guid;
    status = key.readValue("MachineGuid", &machine_guid);
    if (status != ERROR_SUCCESS)
    {
        LOG(ERROR) << "Unable to read machine GUID:" << status;
        return QByteArray();
    }

    return GenericHash::hash(GenericHash::SHA256, machine_guid.toUtf8());
}
