...
bool File::SetTimes(time_t last_access_time, time_t last_modified_time)
{
    DCHECK(IsValid());
...
bool File::GetInfo(Info* info)
{
    ...
    info->last_modified = UnixTimeFromFileTime(file_info.ftLastWriteTime);
    info->last_accessed = UnixTimeFromFileTime(file_info.ftLastAccessTime);
    info->creation_time = UnixTimeFromFileTime(file_info.ftCreationTime);
    return true;
}
