...
int MessagePumpForWin::run(Delegate* delegate)
{
...
void MessagePumpForWin::scheduleWork()
{
    ...
    // tasks/timers while sitting in a nested message loop.
    // Nested loops only look at Windows Message queues, and don't look at *our* task queues, etc.,
    // so we might not get a time slice in such. :-(
...
void MessagePumpForWin::scheduleDelayedWork(const MessageLoopTimePoint& delayed_work_time)
{
    ...
    // are created and destroyed, we update SetTimer.
    // Getting a spurrious SetTimer event firing is benign, as we'll just be processing an empty
    // timer queue.
...
int MessagePumpForWin::runWithDispatcher(Delegate* delegate, MessagePumpDispatcher* dispatcher)
{
    RunState state;
...
int MessagePumpForWin::doRunLoop()
{
...
void MessagePumpForWin::waitForWork()
{
    ...
    for (;;)
    {
        ...
        if (result == WAIT_OBJECT_0)
        {
            ...
            // thread inputs are implicitly attached.
            // This causes the MsgWaitForMultipleObjectsEx API to return indicating that messages
            // are ready for processing (Specifically, mouse messages intended for the child window
            // may appear if the child window has capture).
            // The subsequent PeekMessages call may fail to return any messages thus causing us to
            // enter a tight loop at times.
            // The code below is a workaround to give the child window some time to process its
            // input messages by looping back to MsgWaitForMultipleObjectsEx above when there are
            // no messages for the current thread.
...
bool MessagePumpForWin::processPumpReplacementMessage()
{
    ...
    // This method carefully peeks a message while there is no chance for a kMsgHaveWork to be pending,
    // then resets the have_work_ flag (allowing a replacement kMsgHaveWork to possibly be posted),
    // and finally dispatches that peeked replacement. Note that the re-post of kMsgHaveWork may be
    ...
    // Guarantee we'll get another time slice in the case where we go into native windows code.
    // This scheduleWork() may hurt performance a tiny bit when tasks appear very infrequently, but
    // when the event queue is busy, the kMsgHaveWork events get (percentage wise) rarer and rarer.
    scheduleWork();
