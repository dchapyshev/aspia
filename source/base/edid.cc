...
std::unique_ptr<Edid> Edid::create(std::unique_ptr<uint8_t[]> data, size_t data_size)
{
    ...
    if (data && data_size >= sizeof(Data))
    {
        ...
        if (edid->header == kEdidHeader)
        {
            ...
            DLOG(LS_WARNING) << "Invalid EDID checksum: " << checksum;
        }
        else
        {
            DLOG(LS_WARNING) << "Invalid EDID header: " << edid->header;
        }
    }
    else
    {
        DLOG(LS_WARNING) << "Invalid EDID data";
    }
...
int Edid::weekOfManufacture() const
{
    ...
    {
        DLOG(LS_WARNING) << "Wrong week field value: " << week;
        return 0;
    }
