...
TEST(KeyPair, SessionKey)
{
    ...
    EXPECT_TRUE(side1.privateKey().toByteArray() != side1.publicKey());

    KeyPair side2 = KeyPair::create(KeyPair::Type::X25519);
    EXPECT_TRUE(side2.isValid());
    EXPECT_TRUE(side2.privateKey().toByteArray() != side2.publicKey());

    EXPECT_TRUE(side1.privateKey() != side2.privateKey());
    EXPECT_TRUE(side1.publicKey() != side2.publicKey());

    SecureByteArray session_key1 = side1.sessionKey(side2.publicKey());
    EXPECT_FALSE(session_key1.isEmpty());

    SecureByteArray session_key2 = side2.sessionKey(side1.publicKey());
...
TEST(KeyPair, Convert)
{
    ...
    EXPECT_TRUE(pair1.privateKey().toByteArray() != pair1.publicKey());

    SecureByteArray private_key1 = pair1.privateKey();
    ...
    SecureByteArray private_key2 = pair2.privateKey();
