...
constexpr FeatureBit kFeatures[] =
{
    ...
    { GROUP_INSTRUCTION_SET, "Fast Short REP CMPSB/SCASB (FSRC)", 0x80000021, 0, REG_EAX, 11 },
    ...
    { GROUP_INSTRUCTION_SET, "Fast Short REP STOSB (FSRS)", 0x80000021, 0, REG_EAX, 10 },
    ...
    { GROUP_INSTRUCTION_SET, "INVLPGB / TLBSYNC Instruction", 0x80000008, 0, REG_EBX, 3 },
    ...
    { GROUP_INSTRUCTION_SET, "PCOMMIT Instruction", 0x00000007, 0, REG_EBX, 22 },
    ...
    { GROUP_INSTRUCTION_SET, "PREFETCHI Instruction", 0x80000021, 0, REG_EAX, 20 },
    { GROUP_INSTRUCTION_SET, "PREFETCHWT1 Instruction", 0x00000007, 0, REG_ECX, 0 },
    { GROUP_INSTRUCTION_SET, "PTWRITE Instruction", 0x00000014, 0, REG_EBX, 4 },
    { GROUP_INSTRUCTION_SET, "RAO-INT Instructions", 0x00000007, 1, REG_EAX, 3 },
    ...
    { GROUP_INSTRUCTION_SET, "VIA Alternate Instruction Set", 0xC0000001, 0, REG_EDX, 0 },
    ...
    // Security features.
    //----------------------------------------------------------------------------------------------
    { GROUP_SECURITY, "Advanced Cryptography Engine (ACE)", 0xC0000001, 0, REG_EDX, 6 },
    { GROUP_SECURITY, "Advanced Cryptography Engine 2 (ACE2)", 0xC0000001, 0, REG_EDX, 8 },
    { GROUP_SECURITY, "Automatic IBRS", 0x80000021, 0, REG_EAX, 8 },
    ...
    { GROUP_SECURITY, "Control-flow Enforcement Supervisor Shadow Stack (CET_SSS)",
      0x00000007, 1, REG_EDX, 18 },
    { GROUP_SECURITY, "Enhanced Predictive Store Forwarding (EPSF)", 0x80000021, 0, REG_EAX, 18 },
    ...
    { GROUP_SECURITY, "PadLock Hash Engine (PHE)", 0xC0000001, 0, REG_EDX, 10 },
    { GROUP_SECURITY, "PadLock Montgomery Multiplier (PMM)", 0xC0000001, 0, REG_EDX, 12 },
    { GROUP_SECURITY, "PadLock Random Number Generator (RNG)", 0xC0000001, 0, REG_EDX, 2 },
    ...
    { GROUP_SECURITY, "Secure Memory Encryption (SME)", 0x8000001F, 0, REG_EAX, 0 },
    { GROUP_SECURITY, "SGX Attestation Services (SGX-KEYS)", 0x00000007, 0, REG_EDX, 1 },
    ...
    { GROUP_SECURITY, "Special Register Buffer Data Sampling Control (SRBDS_CTRL)",
      0x00000007, 0, REG_EDX, 9 },
    ...
    { GROUP_POWER, "Connected Standby", 0x80000007, 0, REG_EDX, 13 },
    ...
    { GROUP_POWER, "Hardware Thermal Control (HTC)", 0x80000007, 0, REG_EDX, 4 },
    ...
    { GROUP_POWER, "Running Average Power Limit (RAPL)", 0x80000007, 0, REG_EDX, 14 },
    { GROUP_POWER, "Software Thermal Control (STC)", 0x80000007, 0, REG_EDX, 5 },
    ...
    //----------------------------------------------------------------------------------------------
    { GROUP_VIRTUALIZATION, "AMD Virtual Interrupt Controller (AVIC)", 0x8000000A, 0, REG_EDX, 13 },
    { GROUP_VIRTUALIZATION, "Decode Assists", 0x8000000A, 0, REG_EDX, 7 },
    { GROUP_VIRTUALIZATION, "Encrypted State (SEV-ES)", 0x8000001F, 0, REG_EAX, 3 },
    { GROUP_VIRTUALIZATION, "Flush by ASID", 0x8000000A, 0, REG_EDX, 6 },
    { GROUP_VIRTUALIZATION, "Guest Mode Execute Trap Extension (GMET)",
      0x8000000A, 0, REG_EDX, 17 },
    { GROUP_VIRTUALIZATION, "Hypervisor", 0x00000001, 0, REG_ECX, 31 },
    { GROUP_VIRTUALIZATION, "LBR Virtualization", 0x8000000A, 0, REG_EDX, 1 },
    { GROUP_VIRTUALIZATION, "Memory Bandwidth Enforcement (MBE)", 0x80000020, 0, REG_EBX, 1 },
    { GROUP_VIRTUALIZATION, "MSR-Based TSC Rate Control", 0x8000000A, 0, REG_EDX, 4 },
    { GROUP_VIRTUALIZATION, "Nested Paging (NPT, RVI)", 0x8000000A, 0, REG_EDX, 0 },
    { GROUP_VIRTUALIZATION, "NMI Virtualization (vNMI)", 0x8000000A, 0, REG_EDX, 25 },
    { GROUP_VIRTUALIZATION, "NRIP Save (NRIPS)", 0x8000000A, 0, REG_EDX, 3 },
    { GROUP_VIRTUALIZATION, "PAUSE Filter Threshold", 0x8000000A, 0, REG_EDX, 12 },
    { GROUP_VIRTUALIZATION, "PAUSE Intercept Filter", 0x8000000A, 0, REG_EDX, 10 },
    { GROUP_VIRTUALIZATION, "Secure Encrypted Virtualization (SEV)", 0x8000001F, 0, REG_EAX, 1 },
    { GROUP_VIRTUALIZATION, "Secure Nested Paging (SEV-SNP)", 0x8000001F, 0, REG_EAX, 4 },
    { GROUP_VIRTUALIZATION, "Secure Virtual Machine (SVM, Pacifica)", 0x80000001, 0, REG_ECX, 2 },
    { GROUP_VIRTUALIZATION, "SPEC_CTRL Virtualization", 0x8000000A, 0, REG_EDX, 20 },
    { GROUP_VIRTUALIZATION, "SVM Lock (SVML)", 0x8000000A, 0, REG_EDX, 2 },
    { GROUP_VIRTUALIZATION, "Virtual Machine Extensions (VMX, Vanderpool)",
      0x00000001, 0, REG_ECX, 5 },
    { GROUP_VIRTUALIZATION, "Virtual Transparent Encryption (VTE)", 0x8000001F, 0, REG_EAX, 16 },
    { GROUP_VIRTUALIZATION, "Virtualized GIF (vGIF)", 0x8000000A, 0, REG_EDX, 16 },
    { GROUP_VIRTUALIZATION, "Virtualized VMLOAD and VMSAVE", 0x8000000A, 0, REG_EDX, 15 },
    { GROUP_VIRTUALIZATION, "VMCB Clean Bits", 0x8000000A, 0, REG_EDX, 5 },
    { GROUP_VIRTUALIZATION, "x2APIC Virtualization (x2AVIC)", 0x8000000A, 0, REG_EDX, 18 },
    ...
    { GROUP_CPUID, "Code and Data Prioritization (CDP)", 0x00000010, 1, REG_ECX, 2 },
    ...
    { GROUP_CPUID, "Time Stamp Counter Adjust (TSC_ADJUST)", 0x00000007, 0, REG_EBX, 1 },
    ...
    { GROUP_CPUID, "Upper Address Ignore (UAI)", 0x80000021, 0, REG_EAX, 7 },
