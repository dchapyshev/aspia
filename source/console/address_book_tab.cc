      ...
      password_(password)
{
    ...
    ComputerGroup* group_item =
        new ComputerGroup(QIcon(QStringLiteral(":/icon/folder.png")), &root_group_, nullptr);
...
void AddressBookTab::onGroupItemClicked(QTreeWidgetItem* item, int /* column */)
{
    ...
    bool is_root = !current_item->ParentComputerGroup();
...
{
    ComputerGroup* current_group = reinterpret_cast<ComputerGroup*>(ui.tree_group->currentItem());
    if (!current_group)
    {
    ...
    {
        bool is_root = !current_group->ParentComputerGroup();
        emit computerGroupActivated(true, is_root);
    }

    Computer* current_computer = reinterpret_cast<Computer*>(ui.tree_computer->currentItem());
    if (!current_computer)
...
void AddressBookTab::updateComputerList(ComputerGroup* computer_group)
{
