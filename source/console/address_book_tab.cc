...
AddressBookTab* AddressBookTab::createNew(QWidget* parent)
{
    ...
    {
        LOG(INFO) << "[ACTION] Create new rejected by user";
        return nullptr;
    }

    LOG(INFO) << "[ACTION] Create new accepted by user";
...
{
    LOG(INFO) << "Open address book from file:" << file_path;
    ...
    if (buffer.isEmpty())
    {
        LOG(ERROR) << "Unable to read address book file";
    ...
    {
        LOG(ERROR) << "Unable to parse address book file";
...
void AddressBookTab::addComputerGroup()
{
    ...
    if (!parent_item)
    {
        LOG(ERROR) << "Unable to get parent item";
        return;
    }
    ...
    if (dialog.exec() != QDialog::Accepted)
    {
        LOG(INFO) << "[ACTION] Add computer group rejected by user";
        return;
    }

    LOG(INFO) << "[ACTION] Add computer group accepted by user";
...
void AddressBookTab::addComputer()
{
    ...
    LOG(INFO) << "[ACTION] Add computer accepted by user";
...
void AddressBookTab::copyComputer()
{
    ...
    if (!current_item)
    {
        LOG(ERROR) << "Unable to get current item";
        return;
    }
    ...
    if (dialog.exec() != QDialog::Accepted)
    {
        LOG(INFO) << "[ACTION] Copy computer rejected by user";
        return;
    }

    LOG(INFO) << "[ACTION] Copy computer accepted by user";
...
{
    LOG(INFO) << "[ACTION] Modify address book";
    stopOnlineChecker();

    ComputerGroupItem* root_item = rootComputerGroupItem();
    if (!root_item)
    {
        LOG(ERROR) << "Invalid root item";
        return;
    }
    ...
    if (dialog.exec() != QDialog::Accepted)
    {
        LOG(INFO) << "[ACTION] Address book not modified";
        return;
    }

    LOG(INFO) << "[ACTION] Address book modified";
...
void AddressBookTab::modifyComputerGroup()
{
    ...
    if (!current_item)
    {
        LOG(ERROR) << "Unable to get current item for group";
        return;
    }
    ...
    if (!parent_item)
    {
        LOG(ERROR) << "Unable to get parent item for group";
        return;
    }
    ...
    if (dialog.exec() != QDialog::Accepted)
    {
        LOG(INFO) << "[ACTION] Computer group not modified";
        return;
    }

    LOG(INFO) << "[ACTION] Computer group modified";
...
void AddressBookTab::modifyComputer()
{
    ...
    if (dialog.exec() != QDialog::Accepted)
    {
        LOG(INFO) << "[ACTION] Computer not modified";
        return;
    }

    LOG(INFO) << "[ACTION] Computer modified";
...
{
    LOG(INFO) << "[ACTION] Remove computer group";
    stopOnlineChecker();
    ...
    {
        LOG(INFO) << "[ACTION] Computer group removing rejected by user";
    }
}
...
{
    LOG(INFO) << "[ACTION] Remove computer";
    stopOnlineChecker();
    ...
    {
        LOG(INFO) << "[ACTION] Computer removing rejected by user";
    }
}
...
void AddressBookTab::startOnlineChecker()
{
    ...
    for (int i = 0; i < ui.tree_computer->topLevelItemCount(); ++i)
    {
        ...
        if (!computer_item)
        {
            LOG(ERROR) << "Unable to get computer item:" << i;
            continue;
        }
        ...
        if (!computer)
        {
            LOG(ERROR) << "Unable to get computer:" << i;
            continue;
        }
...
void AddressBookTab::stopOnlineChecker()
{
    if (online_checker_)
    {
        LOG(INFO) << "Destory online checker";
...
{
    LOG(INFO) << "[ACTION] Group item clicked";
...
{
    LOG(INFO) << "[ACTION] Group context menu";
...
{
    LOG(INFO) << "[ACTION] Group item collapsed";

    ComputerGroupItem* current_item = dynamic_cast<ComputerGroupItem*>(item);
    if (!current_item)
    {
        LOG(INFO) << "Unable to get current item";
        return;
    }
...
{
    LOG(INFO) << "[ACTION] Group item expanded";
...
{
    LOG(INFO) << "[ACTION] Group item dropped";
...
{
    LOG(INFO) << "[ACTION] Computer item clicked";
...
{
    LOG(INFO) << "[ACTION] Computer context menu";
...
{
    LOG(INFO) << "[ACTION] Computer item double clicked";
...
void AddressBookTab::keyPressEvent(QKeyEvent* event)
{
    ...
    switch (event->key())
    {
        case Qt::Key_Insert:
        {
            LOG(INFO) << "[ACTION] Insert key pressed";
        ...
        case Qt::Key_F2:
        {
            if (focus_widget == ui.tree_group)
            {
                ...
                LOG(INFO) << "[ACTION] F2 key pressed";
        ...
        case Qt::Key_Delete:
        {
            LOG(INFO) << "[ACTION] Delete key pressed";
            if (focus_widget == ui.tree_group)
                removeComputerGroup();
            else if (focus_widget == ui.tree_computer)
                removeComputer();
        ...
        case Qt::Key_Return:
        {
            ...
            {
                LOG(INFO) << "[ACTION] Enter key pressed";
...
void AddressBookTab::onOnlineCheckerResult(int computer_id, bool online)
{
    ...
    if (!item)
    {
        LOG(ERROR) << "Computer with id" << computer_id << "not found in list";
        return;
    }
...
{
    LOG(INFO) << "Online checked finished";

    QTimer::singleShot(0, this, [this]()
    {
        if (online_checker_)
        {
            LOG(INFO) << "Destory online checked";
            online_checker_.reset();
        }
...
void AddressBookTab::updateComputerList(ComputerGroupItem* computer_group)
{
    if (online_checker_)
    {
        LOG(INFO) << "Destroy online checker";
        online_checker_.reset();
    }
...
bool AddressBookTab::saveToFile(const QString& file_path)
{
    ...
    if (path.isEmpty())
    {
        LOG(INFO) << "File path is empty. Show dialog to save file";
        Settings settings;
    ...
    {
        LOG(ERROR) << "Unable to open file for write";
