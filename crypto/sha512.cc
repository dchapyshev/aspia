...
bool SHA512::Update(const std::string& data)
{
    ...
    const uint8_t* buffer = reinterpret_cast<const uint8_t*>(&data[0]);
...
bool SHA512::Final(std::string& hash)
{
    if (curlen_ >= sizeof(buf_))
        return false;
    ...
    hash.resize(kHashSize);

    // Copy output
    for (int i = 0; i < 8; ++i)
    {
        STORE64H(state_[i], &hash[0] + (8 * i));
    }
...
bool CreateSHA512(const std::string& data, std::string& data_hash)
{
    ...
    if (!sha512.Update(data))
        return false;

    return sha512.Final(data_hash);
}
