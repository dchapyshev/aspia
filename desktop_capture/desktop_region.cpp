...
bool DesktopRegion::Equals(const DesktopRegion& region) const
{
    ...
    while (it1 != rows_.end())
    {
        auto second1 = it1->second;
        auto second2 = it2->second;

        if (it2 == region.rows_.end() ||
            it1->first != it2->first ||
            second1->top != second2->top ||
            second1->bottom != second2->bottom ||
            second1->spans != second2->spans)
        {
            return false;
        }
...
void DesktopRegion::MergeWithPrecedingRow(Rows::iterator row)
{
    ...
    if (row != rows_.begin())
    {
        ...
        auto previous_second = previous_row->second;
        auto row_second = row->second;

        // If |row| and |previous_row| are next to each other and contain the same
        // set of spans then they can be merged.
        if (previous_second->bottom == row_second->top &&
            previous_second->spans == row_second->spans)
        {
...
void DesktopRegion::AddSpanToRow(Row* row, int left, int right)
{
    ...
    RowSpanSet::iterator start = std::lower_bound(row->spans.begin(),
                                                  row->spans.end(),
                                                  left,
                                                  CompareSpanRight);
...
bool DesktopRegion::IsSpanInRow(const Row& row, const RowSpan& span)
{
    ...
    // it's the same span.
    RowSpanSet::const_iterator it = std::lower_bound(row.spans.begin(),
                                                     row.spans.end(),
                                                     span.left,
                                                     CompareSpanLeft);
...
void DesktopRegion::Iterator::UpdateCurrentRect()
{
    ...
    rect_ = DesktopRect::MakeLTRB(row_span_->left,
                                  row_->second->top,
                                  row_span_->right,
                                  bottom);
}
